...
// 用户注册
func (s *adminUserService)CreateUser(data *model.CreateUserReq) error {
	// 输入参数检查
...
// 判断用户是否已经登录
func (s *adminUserService)IsSignedIn(session *ghttp.Session) bool {
...
// 获得用户信息详情
func (s *adminUserService)GetProfile(session *ghttp.Session) (u *model.AdminUser) {
	_ = session.GetStruct(USER_SESSION_MARK, &u)
	return
}

// 获得用户列表
func (s *adminUserService)GetList() (list []*model.AdminUser) {
	list, _ = dao.AdminUser.FindAll()
	return
}

func (s *adminUserService)UpdateUser(data *model.UpdateUserReq) error {
	// 输入参数检查
	if e := gvalid.CheckStruct(data, nil); e != nil {
		return errors.New(e.FirstString())
	}
...
//客服接待：排队中的用户
func  (s *adminUserService)QueueList() (list []*model.User,err error){
	list, err = dao.User.Where("status=2").Order("latest_time asc").All()
	return
}

func (s *adminUserService)ConversationList()(data map[string]interface{},err error) {
	list, err := dao.User.Where("status=3 or status =2").Order("latest_time asc").All()
	if err != nil {
		fmt.Println("err = ", err)
		return
	}
